...
public class HashBasedWordCO {
  ...
  public HashSet<String> getUniqueWords(String text) {
    ...
    for (String sentence : sentences) {
      for (String word : sentence.split("\\s+")) {
        ...
        if (!(word.length() < minWordLen) && !isIgnored) {
          uWords.put(word);
        }
      }
    }
  ...
  public void fillCoMat(String text, String[] keyWords) {
    ...
    // for each word, we check its neighbors
    // and increment the HashCounter for each word neighboring
    for (String sentence : sentences) {
      String[] wordsInSentence = sentence.split("\\s+");
      for (int i = 0; i < wordsInSentence.length; i++) {
        int leftCheck = Math.max(0, (i - windowSize));
        int rightCheck = Math.min((i + windowSize), (wordsInSentence.length - 1));
        if (words.contains(wordsInSentence[i])) {
          for (int j = leftCheck; j < rightCheck + 1; j++) {
            if (i != j) {
              if (words.contains(wordsInSentence[j])) {
                coMat.get(wordsInSentence[i]).increment(wordsInSentence[j]);
              }
            }
          }
        }
    ...
    for (String sentence : sentences) {
      ...
      for (int i = 0; i < wordsInSentence.length; i++) {
        ...
        if (wordsInSentence[i].length() < minWordLen) {
          validBase = false;
        }
        if (validBase) {
          coMat.put(wordsInSentence[i], new HashCounter<>());
          int leftCheck = Math.max(0, (i - windowSize));
          int rightCheck = Math.min((i + windowSize), (wordsInSentence.length - 1));
          for (int j = leftCheck; j < rightCheck + 1; j++) {
            if (i != j) {
              ...
              for (String word : ignoreList) {
                if (wordsInSentence[j].equals(word)) {
                  validCheck = false;
                  break;
                }
              }
              if (wordsInSentence[j].length() < minWordLen) {
                validCheck = false;
              }
              if (validCheck) {
                coMat.get(wordsInSentence[i]).increment(wordsInSentence[j]);
